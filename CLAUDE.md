...
1. **Automatic Type Conversion** (`utils.py`):
...
1. GitHub Actions workflow automatically:
   - Installs uv
...
1. Inherit from `GPXModel`
1. Define `_tag` class attribute for XML tag name
1. Define fields with type annotations:
...
1. The `from_xml()` and `to_xml()` methods are inherited from `GPXModel`
1. Add custom types to `types.py` if needed
1. Export from `src/gpx/__init__.py`
1. Follow the [Change Management Requirements](#change-management-requirements) checklist:
